...
- [x] 素材別の状況を詰まった区切り線レイアウトから、余白を持つ小カード構成へ整理する。
- [x] 素材別の状況に、対象数、処理済み、Timeline 登録、言語化またはイベント数を表示する。
- [x] 成長バーの静的スタイルを Tailwind クラスへ寄せ、動的幅だけをインライン style に残す。
...
- 2026-05-11 に素材別の状況を4指標表示へ変更し、`dotnet.exe build web/Timeline.Web.csproj` が通ることを確認。
- 2026-05-11 に成長バーの静的スタイルをクラス化し、`dotnet.exe build web/Timeline.Web.csproj` が通ることを確認。
