...
Timeline の製品設定には GitHub リポジトリ URL を保存し、インストールまたは更新時に最新タグを確認して対応する source archive ZIP を取得します。ローカル helper は公開タグフィードを優先して読み、必要な場合だけ GitHub API にフォールバックするため、通常利用で GitHub アカウントや Git は不要です。
