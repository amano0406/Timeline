...
- 設定
  - 通常モード: 表示言語、タイムゾーン、Timeline 保存先、AI設定、入力データ
  - プロモード: Timeline 本体と各製品の細かい設定
- TimelineForAudio
  - 音声ファイル一覧、分析状態、生成物削除、ダウンロード、音声詳細
- TimelineForWindowsCodex
  - スレッド一覧、スレッド詳細、生成物削除、ダウンロード
- TimelineForChatGPT
  - ZIPアップロード、スレッド一覧、スレッド詳細、生成物削除、ダウンロード
- TimelineForImage
  - 画像ファイル一覧、再スキャンして分析、生成物削除、ダウンロード
...
画面の「タイムライン」では、製品横断の時間軸を再構築して一覧表示します。再構築時は、各製品から `cli.ps1` 経由で Timeline 保存先配下の作業領域へダウンロードし、その後 Timeline 側で `items.jsonl` と `events.jsonl` に正規化します。
...
画面では Timeline 保存先だけを選択します。作業データ、時間軸ストア、操作ログは保存先の配下で Timeline が管理します。
